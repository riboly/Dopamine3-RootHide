...
- `3.0.32` adds an iOS 18 complete-path no-injection rule for Apple's `/usr/libexec/neagent` while preserving third-party extension handling. On 2026-08-26 the target device completed activation and multiple VPN tools connected normally, so the NetworkExtension fix is `DEVICE VERIFIED`. The version also replaces the network-downloaded Frida package with bundled `frida_16.3.3_RootHides-arm64e.deb` (`re.frida.server` 16.3.3, `iphoneos-arm64e`, SHA-256 `e1b48522fa28c512b875643fb58155ab655b6c5cf3e9590323eb371ed1dc3970`); that installer path remains `BUILD VERIFIED` until its settings action is tested on-device. Actions run `32921982914` produced `roothide-Dopamine-3.0.32-a2619ac.tipa`.
