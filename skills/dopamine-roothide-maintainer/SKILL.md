...
- `3.0.33` is the device-verified stable baseline for the restored network Frida installer and manually dispatchable full-build workflow. Manual Actions run `32981228809` completed successfully and produced `roothide-Dopamine-3.0.33-ed8387a.tipa`. The 2026-08-27 `thermalmonitord` userspace panics were caused by the subsequently removed `byg.iosios.net.monitor` 1.5.7-7 tweak, not by an unexplained 3.0.33 core regression.
- `3.0.34` protects exactly `/usr/libexec/thermalmonitord` from injection on iOS 18 and adds a bounded watchdog-created quarantine for reviewed dedicated Apple service paths. Full Actions run `33140458314` produced build-verified artifact `roothide-Dopamine-3.0.34-308db3c.tipa`. It is not device verified until activation, normal thermal behavior, package-manager regressions, and at least one full reboot/reactivation cycle pass.
...
16. A watchdog-created no-injection quarantine must be evidence gated: require the literal `induced crashes` text, map an exact watchdog service name to a reviewed complete Apple executable path, and persist only that path. Do not include SpringBoard, `extensionkitservice`, or `watchdogd`; do not let quarantine disable the watchdog protection chain. Keep fixed exclusions separate from removable quarantine state. A global dpkg/Sileo interceptor is not an acceptable substitute because it creates a package-management single point of failure and cannot reliably infer runtime injection behavior from every package format.
17. Cache spawn-path settings with a short bounded TTL and thread-safe scalar state. Preserve live settings changes; never replace every-spawn XPC with a permanent cache. The 4 GB device baseline uses a 1.5x Jetsam default while keeping the user-selectable 1x-4x range.
18. Increment both `Application/Makefile` and `BaseBin/_external/basebin/.version` together.
19. Run `git diff --check`, inspect the complete diff, then build through `.github/workflows/roothide.yml`.
20. Verify the downloaded artifact digest, TIPA integrity, app/basebin versions, required Mach-O slices, and any new runtime marker before installation.
21. Validate on the device. For dynamic trust persistence, seed each pre-existing tool once after upgrading, record target CDHashes and the registry before reboot, then prove those hashes are restored before the tools or affected apps are launched after reboot. A CDHash absent from both the registry and live cache was never persisted and requires one successful tool registration before this test.
22. Update the maintenance document with the confirmed result, release artifact, commit, and remaining risks.
