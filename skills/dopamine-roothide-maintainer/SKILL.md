...
17. Validate on the device. For dynamic trust persistence, seed each pre-existing tool once after upgrading, record target CDHashes and the registry before reboot, then prove those hashes are restored before the tools or affected apps are launched after reboot. A CDHash absent from both the registry and live cache was never persisted and requires one successful tool registration before this test.
