...
- `3.0.34` is device-rejected: after installation the phone became severely janky and Wi-Fi/VPN sessions repeatedly disconnected and reconnected. Its shared critical-service file cache and broad watchdog quarantine are intentionally rolled back.
- `3.0.35` returns to the 3.0.33 runtime design and adds only an iOS 18 complete-path no-injection rule for `/usr/libexec/thermalmonitord`, addressing the observed `no successful checkins ... (2 induced crashes)` after `byg.iosios.net.monitor` 1.5.7-7 without adding runtime file I/O or automatic quarantine state. It is not device verified until build and reboot-cycle testing pass.
...
16. If a third-party CPU/thermal tweak causes `thermalmonitord` watchdog check-in failures, the conservative fix is a static complete-path no-injection rule for `/usr/libexec/thermalmonitord` in both spawn-config implementations. Do not add per-spawn filesystem reads, broad watchdog quarantine, service termination, or global package-manager interception for this symptom; those mechanisms can add latency and destabilize Wi-Fi/VPN indirectly.
